...
macro_rules! db_conn {
    ($var:ident) => {
        let $var = $crate::db::open_db().expect("Failed to open database");
    };
}
#[macro_export]
macro_rules! db_exec {
    ($conn:expr, $query:expr) => {
        $crate::db::db_exec($conn, $query, &[]).ok()
    };
    ($conn:expr, $query:expr, $($params:expr),+) => {
        $crate::db::db_exec($conn, $query, &[$($params),+]).ok()
    };
}
#[macro_export]
macro_rules! db_query {
    ($conn:expr, $query:expr) => {
        $crate::db::db_query($conn, $query, &[])
    };
    ($conn:expr, $query:expr, $($params:expr),+) => {
        $crate::db::db_query($conn, $query, &[$($params),+])
    };
}
#[macro_export]
macro_rules! http_server {
    ($port:expr) => {
        $crate::web::web_start($port);
    };
}
#[macro_export]
macro_rules! route {
    ($path:expr, $handler:expr) => {
        $crate::web::web_route($path, $handler);
    };
}
#[macro_export]
macro_rules! jwt_sign {
    ($user_id:expr, $hours:expr, $data:expr) => {
        $crate::auth::auth_jwt_create($user_id, $hours, $data)
    };
}
#[macro_export]
macro_rules! jwt_verify {
    ($token:expr) => {
        $crate::auth::auth_jwt_verify($token)
    };
}
#[macro_export]
macro_rules! password_hash {
    ($password:expr) => {
        $crate::auth::auth_hash_password($password)
    };
}
#[macro_export]
macro_rules! password_verify {
    ($password:expr, $hash:expr) => {
        $crate::auth::auth_verify_password($password, $hash)
    };
}
